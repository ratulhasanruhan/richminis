...
<div class="aiz-card-box h-auto bg-white py-3">
    <div class="position-relative h-100px h-md-200px img-fit overflow-hidden">
        ...
        @endphp
        <!-- Image -->
        <a href="{{ $product_url }}" class="d-block mx-auto w-100px w-sm-auto h-100 position-relative image-hover-effect rounded-2 ">
            <img
                ...
                <div class="d-inline-flex px-2 py-1 shadow-sm">
                    ...
                    @if ( (is_array($colors) && count($colors) > 0) || (is_array($attributes) && count($attributes) > 0) )
                        <a href="javascript:void(0)" class="hov-svg-white d-inline-block mb-2"
                            onclick="showAddToCartRightCanvas({{ $product->id }})" data-toggle="tooltip"
                            data-title="{{ translate('Select Variant') }}" data-placement="top">
